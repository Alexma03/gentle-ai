...
func commandRunWith(args []string, isExecutable func(string) bool, journeys func() []Journey) int {
	...
	results.Notes = []string{
		...
		"That HOME is a fresh personal install, so receipt-driven development defaults on. Every journey still declares whether the runner ensures that state or leaves the switch untouched for the journey itself.",
