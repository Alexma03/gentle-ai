...
- **PR rule**: review can provide fresh evidence for a commit, push, or PR, but it never authorizes delivery. Receipt-driven development is on by default in this personal fork and can be disabled globally or per clone; ordinary repository policy always decides delivery.
