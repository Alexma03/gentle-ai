...
// ConfiguredAgentDir returns the agent-owned config directory selected by Pi's
// runtime. The CodeGraph component uses this projection while retaining all
// discovery and precedence policy in its own package.
func ConfiguredAgentDir(homeDir string) string {
	if configured := strings.TrimSpace(os.Getenv(piCodingAgentDirEnv)); configured != "" {
		return configured
	}
	return AgentConfigPath(homeDir)
}

// ConfiguredMCPPath returns the MCP configuration path below a Pi agent
// directory. It is a client path projection, not CodeGraph discovery policy.
func ConfiguredMCPPath(agentDir string) string {
	return filepath.Join(agentDir, piEngramMCPConfigFile)
}
