...
// OpenCodeBackgroundIntent preserves the retired persisted auto/on/off field
// so migration and rollback can round-trip old state without activating it.
...
// ParseOpenCodeBackgroundIntent is intentionally absent: the compatibility
// value may be decoded and restored, but it is no longer user-selectable.

// PiBackgroundIntent is Pi's unresolved auto/on/off background-subagent
// preference. It remains independent from retired compatibility state.
...
const (
	...
	// StrategyFileReplace replaces an entire managed system-prompt file.
	StrategyFileReplace
...
const (
	...
	// for clients whose settings own that shape.
...
// SDDProfileStrategyID preserves retired profile-strategy state for rollback.
type SDDProfileStrategyID string

const (
	// SDDProfileStrategyGeneratedMulti is the default/backward-compatible mode:
	// named profiles were generated by older releases.
	...
	// that kept profile state outside the main settings document.
