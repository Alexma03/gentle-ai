...
func RenderRouting(agent model.AgentID) (string, error) {
	...
	output.WriteString("- It is **on by default in this personal fork**. An explicit global or clone-local `off` always wins, and delivery then follows ordinary repository policy.\n")
	output.WriteString("- `status` is read-only. It reports the deciding source and the effective mode, and changes nothing. A `default` deciding source means nobody recorded an override, so the personal default is on.\n")
	...
	output.WriteString("- Never reverse an explicit disable or enable receipt-driven development on the user's behalf after they turn it off.\n")
