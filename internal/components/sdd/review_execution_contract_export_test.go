...
func TestReviewExecutionContractForPiMatchesBoundOrchestratorContract(t *testing.T) {
	...
	if !strings.Contains(got, "## Atomic lifecycle") {
		t.Fatal("ReviewExecutionContractFor(pi) is missing the personal fork's `## Atomic lifecycle` heading")
	}
}
...
func TestReviewExecutionContractForPiUsesFacadeLifecycleRoutes(t *testing.T) {
	...
	if strings.Contains(contract, "answer-consent") || strings.Contains(contract, "consentBinding") {
		t.Fatal("Pi review contract reintroduced candidate-scoped consent")
	}
}
