...
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use. Then pass to `sdd-apply`: implement only the next autonomous natural work-unit slice using work-unit commits, with clear start, finish, verification, and rollback boundary.
