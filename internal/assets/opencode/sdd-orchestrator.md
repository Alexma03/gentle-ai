...
When the native route is representable, ask all three preflight groups in one single `question` tool call so OpenCode can render the groups as tabs. Do NOT run this as a sequential wizard. Do NOT issue three separate `question` tool calls.
...
The PR canonical values are exactly the `delivery_strategy` domain `sdd-tasks` and `sdd-apply` accept; never emit a value outside it. The preflight offers no separate chained option because chaining is selected only when the qualitative task forecast identifies natural review boundaries.
...
This is collected by `SDD Session Preflight` as the chained PR strategy. If missing, enforce the hard gate before any phase work; changed-line counts never influence the decision. Ask which delivery/review strategy they want:

- **`ask-on-risk`** (default): Ask later only when `sdd-tasks` identifies a genuine qualitative workload decision.
- **`auto-chain`**: Continue with chained/stacked PR slices at natural architectural or review boundaries without asking again.
- **`single-pr`**: Preserve one PR when the plan explains why the work remains cohesive and reviewable.

These three are the whole domain. Cache the delivery strategy for the session. Pass it as `delivery_strategy` to `sdd-tasks` and `sdd-apply` prompts.
...
If it says `Chained PRs recommended: Yes` or `Decision needed before apply: Yes`, apply the cached `delivery_strategy`. Whenever a directive below tells the orchestrator to ask the user a decision (natural split vs. one cohesive PR, or which chain strategy), use one `question` tool call only when the complete decision is natively representable; otherwise emit the complete choice through the plain chat or terminal fallback and STOP.

- **`ask-on-risk`**: STOP and ask whether to use the proposed natural split or proceed as one cohesive PR, using the lossless blocking-prompt route. If the user chooses chaining and `chain_strategy` is not yet cached, ask which chain strategy to use (stacked-to-main or feature-branch-chain) through the same route.
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use through the lossless blocking-prompt route. Then pass to `sdd-apply`: implement only the next autonomous natural work-unit slice using work-unit commits, with clear start, finish, verification, and rollback boundary.
...
When launching `sdd-apply`, always include the resolved `delivery_strategy`, `chain_strategy`, and chosen natural PR boundary in the prompt.
