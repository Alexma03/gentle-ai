...
On the first `/sdd-new`, `/sdd-ff`, or `/sdd-continue` (or an equivalent natural-language request) in a session, ask once for and cache delivery strategy: `ask-on-risk` (default), `auto-chain`, or `single-pr`. Pass it as `delivery_strategy` to `sdd-tasks` and `sdd-apply` context. Review workload is qualitative; changed-line counts never influence the decision.
...
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use. Then apply only the next autonomous natural work-unit slice using work-unit commits, clear start/finish boundaries, verification, and rollback.
