...
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use. Then run `sdd-apply` inline for only the next autonomous natural work-unit slice using work-unit commits, clear start/finish boundaries, verification, and rollback.
