...
- Every journey declares `Review:` — `reviewOptedIn` (the runner ensures receipt-driven development is globally enabled before the first step, uncounted, and fails the journey if the switch is not on) or `reviewUntouched` (its subject IS the switch, or it has nothing to do with reviews). The declaration is mandatory; `validateCorpus` fails the run without it. Fresh personal installs default RDD on, but journeys must still declare whether the runner should enforce that state or leave it untouched.
...
2. Author or adapt the journey; update its title, step names, and comment to say WHY the expectation holds (cite the ratified decision or concrete regression evidence).
...
Delivery evidence includes the driven-mode summary line (completed / unsupported / failed counts) from a locally built binary, not only `go test` output.
