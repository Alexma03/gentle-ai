...
func runSDDAttempt(ctx context.Context, args []string, stdout io.Writer) error {
	...
	*remediatesEvidenceRevision = strings.TrimSpace(*remediatesEvidenceRevision)
	...
	if (operation == "finish" || operation == "settle") && declaredUntracked {
		if shapeErr := intendedUntrackedDeclarationShape(untrackedScope, intendedUntracked, expectedUntrackedInventory, expectedUntrackedInventory.value, "gentle-ai review status --next-transition", "gentle-ai sdd-attempt "+operation); shapeErr != nil {
			return shapeErr
		}
