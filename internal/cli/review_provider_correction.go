...
// exit being a relaunch of the same slot that failed the same way. The runtime
// never learned what was wrong. The capture now grants exactly one corrective
// re-invocation whose prompt is the original materialized prompt plus the exact
// admission error and the census of what remains invalid. The bound is a named
// constant, the retry lives only here, and it never applies to --input
// submissions or a host relay: those hosts own their reviewer and receive the
// same preserved payload instead.
const (
