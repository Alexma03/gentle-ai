	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := RunReview([]string{
				"status", "--cwd", repo, "--contract", ReviewIntegrationContractV2, "--agent", "codex",
			...
			startErr := RunReview([]string{
				"start", "--cwd", repo, "--contract", ReviewIntegrationContractV2, "--agent", "codex",
...
func TestDisabledStatusStaysSilentForEveryCWDSpelling(t *testing.T) {
	for _, cwd := range [][]string{{"--cwd=."}, {"--cwd", "."}} {
		t.Run(strings.Join(cwd, " "), func(t *testing.T) {
			...
			args = append(args, "--contract", ReviewIntegrationContractV2, "--agent", "codex", "--next-transition")
