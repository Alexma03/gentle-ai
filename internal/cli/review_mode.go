...
// reviewStartConsentMode is retained for command compatibility. Current
// provider-issued starts do not request candidate consent: enabling RDD is the
// user's durable authorization to review future candidates automatically.
type reviewStartConsentMode string

const (
	reviewConsentModeNone reviewStartConsentMode = ""
	// reviewConsentModeRelay is accepted for clients generated by older builds.
	// It now behaves like an automatic start when RDD is enabled.
...
// authorizeReviewStart applies the user-owned RDD switch. Enabled means review
// future candidates automatically; disabled refuses review start. The old
// relay and granted declarations remain accepted for client compatibility but
// no longer introduce a second, candidate-scoped consent ceremony. An explicit
// declined declaration remains a one-candidate escape hatch for old clients.
