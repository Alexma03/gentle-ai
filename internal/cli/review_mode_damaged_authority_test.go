...
func TestKillSwitchStatusIsReadableWhileReviewAuthorityIsDamaged(t *testing.T) {
	...
	// No clone-local override was ever written, so the personal default decides
	// and receipt-driven development stays on. What matters
	...
	if result := decodeReviewModeResult(t, output.Bytes()); result.Status.Effective != reviewtransaction.RDDModeOn {
		t.Fatalf("status = %#v, want the personal default to still decide on", result.Status)
	}
}
